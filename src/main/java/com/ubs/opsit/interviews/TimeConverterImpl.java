...
public class TimeConverterImpl implements TimeConverter{
	...
	/**
	 * To convert timeString into Berlin Clock format.
	 * @return berlinTimeString
	 */
	public String convertTime(String timeString) {
		verifyTimeStringFormat(timeString);
		int[] time = convertTimeStringToLocalTime(timeString);
		berlinClock = buildBerlinClock(time);
		return berlinClock.toString();
	}
	
	private int[] convertTimeStringToLocalTime(String timeString) {
	...
	private void verifyTimeStringFormat(String timeString) {
		if (Objects.isNull(timeString) || timeString.isEmpty()) {
